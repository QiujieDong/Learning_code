...
if train:
    train_dataset = HumanSegOrigDataset(dataset_path, train=True, k_eig=k_eig, use_cache=True,
...
def test():
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            # visualize the predicted face
            if os.path.splitext(mesh_file.split('/')[-1])[0] == '12_fix_orientation':
                continue
            if int(os.path.splitext(mesh_file.split('/')[-1])[0]) > 20:
                diffusion_net.utils.visual_face_label(mesh_file, pred_labels, max_label=8)
