    ...
    def __init__(self, root_dir, train, k_eig=128, use_cache=True, op_cache_dir=None):
        ...
        if use_cache:
            ...
            if os.path.exists(load_cache):
                ...
                self.verts_list, self.faces_list, self.frames_list, self.massvec_list, self.L_list, self.evals_list, self.evecs_list, self.gradX_list, self.gradY_list, self.labels_list, self.mesh_files = torch.load(
                    load_cache)
                return
        ...
        if self.train:
            ...
            for _, iShrec in enumerate([30, 40]):
                for i in range(0, 6):
                    file_name = iShrec + i
                    mesh_fname = "{}.ply".format(file_name)
                    label_fname = "shrec__{}.eseg".format(iShrec)
                    mesh_fullpath = os.path.join(mesh_dirpath, mesh_fname)
                    label_fullpath = os.path.join(label_dirpath, label_fname)
                    self.mesh_files.append(mesh_fullpath)
                    label_files.append(label_fullpath)
        ...
        for iFile in range(len(self.mesh_files)):
            print("loading mesh " + str(self.mesh_files[iFile]))

            verts, faces = pp3d.read_mesh(self.mesh_files[iFile])
            if os.path.splitext(self.mesh_files[iFile])[-1] in ['.ply']:
                labels = np.loadtxt(label_files[iFile]).astype(int)
            else:
        ...
        if use_cache:
            ...
            torch.save((self.verts_list, self.faces_list, self.frames_list, self.massvec_list, self.L_list,
                        self.evals_list, self.evecs_list, self.gradX_list, self.gradY_list, self.labels_list, self.mesh_files),
                       load_cache)
    ...
    def __getitem__(self, idx):
        return self.verts_list[idx], self.faces_list[idx], self.frames_list[idx], self.massvec_list[idx], self.L_list[
            idx], self.evals_list[idx], self.evecs_list[idx], self.gradX_list[idx], self.gradY_list[idx], \
               self.labels_list[idx], self.mesh_files[idx]
