...
from kaolin.metrics.point import directed_distance as directed_distance  # 有向距离
...
def trianglemesh_to_pointcloud(mesh: kaolin.rep.Mesh, num_points: int):
    ...
    points, face_choices = mesh.sample(num_points)  # mesh上进行采样
...
def trianglemesh_to_voxelgrid(mesh: kaolin.rep.Mesh, resolution: int,
    ...
    if normalize:  # 归一化
        ...
        mesh.vertices = (mesh.vertices - verts_min) / (verts_max - verts_min) - 0.5  # [-0.5, 0.5]

    mesh.vertices = mesh.vertices + vertex_offset  # vertex_offset:顶点偏移量
    ...
    v1 = torch.index_select(mesh.vertices, 0, faces[:, 0])  # 从第0维中搜索faces[:, 0]中的数据，返回搜索到的数据
