...
def _convert_categories(categories):
    ...
    synsets = [label_to_synset[c] if c in label_to_synset.keys()  # 拿到对应类别的synset
...
class ShapeNet_Meshes(data.Dataset):
    ...
    def __init__(self, root: str, categories: list = ['chair'], train: bool = True,
        ...
        for i in range(len(self.synsets)):
            ...
            models = sorted(class_target.glob('*'))  # sorted()排序操作，默认升序
            stop = int(len(models) * split)  # train:test=7:3
            if train:
                models = models[:stop]
            else:
                models = models[stop:]
