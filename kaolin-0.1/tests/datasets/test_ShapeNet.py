...
def test_Meshes():  # 这个只是测试的Mesh数据中的顶点
    meshes1 = shapenet.ShapeNet_Meshes(root=SHAPENET_ROOT,
                                       categories=['can'], train=True, split=.7)
    assert len(meshes1) > 0  # 确保是有数据的
    for mesh in meshes1:
        assert Path(mesh['attributes']['path']).is_file()  # 判断是否为文件
        assert mesh['data']['vertices'].shape[0] > 0  # 顶点的个数是大于0的
...
@pytest.mark.skipif(not Path(SHAPENET_ROOT).exists(), reason=SHAPENET_NOT_FOUND)
def test_Surface_Meshes():  # 测试mesh数据中的surface数据
    ...
    for smesh in surface_meshes:
        ...
        assert smesh['data']['faces'].shape[1] == 3  # 一个face是由三个点组成

    shutil.rmtree('tests/datasets/cache/surface_meshes')  # shutil是一个文件操作的高级目录，rmtree删除一个目录
