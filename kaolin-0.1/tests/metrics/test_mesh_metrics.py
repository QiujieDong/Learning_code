...
def test_chamfer_distance(device='cpu'):  # 倒角距离
...
def test_laplacian_loss(device='cpu'):  # 拉普拉斯算子损失
    mesh1 = TriangleMesh.from_obj('tests/model.obj')
    mesh2 = TriangleMesh.from_obj('tests/model.obj')
    if device == 'cuda':
        mesh1.cuda()
...
def test_point_to_surface(device='cpu'):
    torch.manual_seed(1)  # 设计随机化初始种子，使初始化的权重是一样的
    torch.cuda.manual_seed(1)  # 给GPU设置随机化初始种子
    mesh = TriangleMesh.from_obj('tests/model.obj')
    points = torch.rand(500, 3) - .5  # 随机生成500行3列的数据
    if device == 'cuda':
        mesh.cuda()
        points = points.cuda()
