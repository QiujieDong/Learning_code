...
def test_to_trianglemesh_to_pointcloud(device):  # 三角mesh转成点云
...
@pytest.mark.parametrize('device', ['cpu', 'cuda'])
def test_trianglemesh_to_voxelgrid(device):  # 三角mesh转成体素
    mesh = TriangleMesh.from_obj('tests/model.obj')
    if device == 'cuda':
        mesh.cuda()
...
@pytest.mark.parametrize('device', ['cpu', 'cuda'])
def test_trianglemesh_to_sdf(device):  # 三角mesh转成SDF
    mesh = TriangleMesh.from_obj('tests/model.obj')
    if device == 'cuda':
        mesh.cuda()
    print(mesh.device)
