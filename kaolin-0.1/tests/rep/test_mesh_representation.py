...
def test_load_obj(device='cpu'):  # 测试已经load进obj
...
def test_laplacian_smoothing(device='cpu'):  # 拉普拉斯平滑
    mesh = TriangleMesh.from_obj(('tests/model.obj'))
    if device == 'cuda':
        mesh.cuda()
...
def test_compute_laplacian(device='cpu'):  # 计算拉普拉斯
    mesh = TriangleMesh.from_obj(('tests/model.obj'))
    if device == 'cuda':
        mesh.cuda()
...
def test_load_and_save_Tensors(device='cpu'):  # 测试将mesh的obj文件保存和加载npz文件
...
def test_adj_computations(device='cpu'):  # 邻接矩阵
    mesh = TriangleMesh.from_obj(('tests/model.obj'))
    if device == 'cuda':
        mesh.cuda()

    adj_full = mesh.compute_adjacency_matrix_full()
    adj_sparse = mesh.compute_adjacency_matrix_sparse().coalesce()  # 邻接离散矩阵
