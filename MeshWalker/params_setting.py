...
def set_up_default_params(network_task, run_name, cont_run_number=0):
    ...
    params.logdir = utils.get_run_folder(
        params.run_root_path + '/', '__' + run_name, params.cont_run_number)
    ...
    if params.network_task == 'classification':
        ...
        raise Exception('Unsuported params.network_task: ' +
                        params.network_task)
...
def shrec11_params(split_part):
    ...
    params = set_up_default_params(
        'classification', 'shrec11_' + split_part, 0)
    params.n_classes = 30
    params.seq_len = 100
    params.min_seq_len = int(params.seq_len / 2)

    params.datasets2use['train'] = [
        'datasets_processed/shrec11/' + split_part + '/train/*.npz']
    params.datasets2use['test'] = [
        'datasets_processed/shrec11/' + split_part + '/test/*.npz']
