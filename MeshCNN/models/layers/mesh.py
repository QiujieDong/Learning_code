...
class Mesh:
    ...
    def clean(self, edges_mask, groups):
        ...
        new_indices[edges_mask] = np.arange(
            0, np.ma.where(edges_mask)[0].shape[0])
    ...
    def export(self, file=None, vcolor=None):
        if file is None:
            if self.export_folder:
                ...
                file = '%s/%s_%d%s' % (self.export_folder,
                                       filename, self.pool_count, file_extension)
            else:
                return
        faces = []
        ...
        new_indices[self.v_mask] = np.arange(
            0, np.ma.where(self.v_mask)[0].shape[0])
        ...
        with open(file, 'w+') as f:
            for vi, v in enumerate(vs):
                vcol = ' %f %f %f' % (
                    vcolor[vi, 0], vcolor[vi, 1], vcolor[vi, 2]) if vcolor is not None else ''
                f.write("v %f %f %f%s\n" % (v[0], v[1], v[2], vcol))
    ...
    def export_segments(self, segments):
        ...
        for i in range(self.pool_count + 1):  # range()不包含stop数
            # os.path.splitex()文件名与扩展名分开，os.path.split()路径与文件名分开
            filename, file_extension = os.path.splitext(self.filename)
            # 更改存储路径，但是不更改文件名
            file = '%s/%s_%d%s' % (self.export_folder,
                                   filename, i, file_extension)
                ...
                with open(file) as old_file:  # 打开file文件
                    for line in old_file:  # 按行读取old_file中的内容
                        if line[0] == 'e':  # 'e'代表本行数据是mesh的'edge'数据
                            ...
                            # 如果不是edge,那么new_file就把old_file中line数据copy过来
                            new_file.write(line)
            os.remove(file)  # 删除file文件，也就是旧的file文件
            # shutil.move()#将abs_path文件移动到file文件位置处，并且保持file的名字
            move(abs_path, file)
            if i < len(self.history_data['edges_mask']): # 变成与第i行一样的行数和列数
                cur_segments = segments[:len(
                    self.history_data['edges_mask'][i])]
    ...
    def __cycle_to_face(self, cycle, v_indices):
        face = []
        for i in range(3):
            v = list(set(self.edges[cycle[i]]) & set(
                self.edges[cycle[(i + 1) % 3]]))[0]
    ...
    def union_groups(self, source, target):
        if self.export_folder and self.history_data:
            self.history_data['collapses'].union(
                self.history_data['current2old'][source], self.history_data['current2old'][target])
        return
